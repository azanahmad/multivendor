...
trait ProductVarientTrait {

    public function option1($product){
        if(count($product->varients) > 0){
            $array = array_unique($product->varients->pluck('Option1')->toArray());
            if($array[0] == ""){
                $array =[];
            }
            return $array;
    ...
    public function option2($product){
        if(count($product->varients) > 0){
            $array =  array_unique($product->varients->pluck('Option2')->toArray());
            if($array[0] == ""){
                $array =[];
            }
            return $array;
    ...
    public function option3($product){
        if(count($product->varients) > 0){
            $array =  array_unique($product->varients->pluck('Option3')->toArray());
            if($array[0] == ""){
                $array =[];
            }
            return $array;
